...
//
//  ColorPick_OnButtonUp
//
//      When the button comes up, we are done.
//
void ColorPick_OnButtonUp(COLORPICKSTATE* pcps, HWND hwnd, int x, int y)
{
...
//  ColorPick_OnKeyDown
//
//      If ESC, Ctrl, Alt or Windows key is pressed, then abandon the fake menu.
//      (This behavior makes it impossible to capture the fake menu with
//      Win+PrtScr and Alt+PrtScr. Only PrtScr is availble.)
...
void ColorPick_OnKeyDown(COLORPICKSTATE* pcps, HWND hwnd, WPARAM vk)
{
    ...
    switch (vk)
    {
    // Same behavior as native menus
    case VK_ESCAPE:
    case VK_MENU:
        pcps->fDone = TRUE;         // Abandoned
        break;
    // Since we close the menu on losing focus, we might freeze the main window
    // on some circumstances, e.g. menu -> Win -> menu -> Win -> menu
    // (You can remove the following `case`s and see what happens.)
    // FIXME: Different from native menus
    case VK_CONTROL:
    case VK_LWIN:
    case VK_RWIN:
        pcps->fDone = TRUE;         // Abandoned
...
// client rect of that size.
    ...
    while (GetMessage(&msg, NULL, 0, 0))
    {
        ...
        // (Win+Tab is not handled here. We have to handle Windows key messages
...
LRESULT CALLBACK FakeMenuDemo_WndProc(HWND hwnd, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CONTEXTMENU:
    {
        ...
        // User presses mouse right button inside the client area,
        // or presses keyboard menu button
        if((point.x >= 0 && point.y >= 0) || (x == -1 && y == -1))
        {
            FakeMenuDemo_OnContextMenu(hwnd, x, y);
            return 0;
        }
        // User presses the mouse right button in the non-client area, e.g. the title bar
        // Won't handle this message so that right-click on the title bar shows
        // the control menu instead of the color picker
        else
        {
            break;
        }
